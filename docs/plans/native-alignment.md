...
**All design items are now decided.** Remaining work is implementation per the
Sequencing section.
...
2. **Per-skill fan-out into `~/.codex/skills`:** confirm Codex loads per-skill
   symlinks under `~/.codex/skills/<name>` the same way it loads real dirs, and
   that mixing roborepo-managed symlinks with native-installed real dirs in the
   same parent is safe. (Riskiest assumption in item 0 — test before committing.)
3. **Per-skill link mechanism (item 0.2):** generated per-skill `manifest.tsv`
...
4. **Claude dir-level → per-skill migration:** does dropping the dir-level
   `~/.claude/skills` symlink and the intermediate `globals/claude/skills/` dir in
   favor of direct `~/.claude/skills/<name>` links break any current doctor/verify
   check that expects the dir-level link? Audit `verify-content.tsv` and
   `doctor.sh` skill checks.

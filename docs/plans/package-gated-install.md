...
deleted.
- `main.sh`: delete `choose_install_mode` (the managed-vs-adopt prompt) and
...
- `presets.mjs` `readInstallPolicy`: drop `mode` (always adopt); keep `onConflict` (default
  `keep`). Delete the **managed** branches in `applyRow` / `bundleApplied` so the
  copy/conflict-reconcile path is the only path.
- `state-lib.sh` + `install-state.json`: drop `mode`; keep `onConflict`.
- Sweep `repair.sh`, `uninstall.sh`, `sync-from-home.sh`, `install-*.sh`,
...
- Add the enabled-packages registry (new state file + read/write helpers).
- Render home `CLAUDE.md` / `AGENTS.md` from `base + enabled-package fragments` (Node core,
  reusing the concatenation logic) on install/enable/disable/update.
- Package `rules` component contributes a fragment to the render; delete the
  `mergeRules`/`unmergeRules` append path.
- `config.mjs`: compute package "active" from the registry.
- Onboarding (`presets.mjs`): write the registry, then re-render once.
...
- Base links only `roborepo-support`; all other skills gated by onboarding.
- Remove the unconditional `roborepo mcp apply` from `main.sh`; MCP registers only on
  package enable.
- Delete `globals/rules/shared/10-exploration.md`; package `rules.md` is the single source.
- Remove the jcodemunch SessionStart message + Grep/Glob hook from base `settings.json`;
  package `hooks-claude.json` is canonical.

### Phase 5 — Explicit update, docs, tests
- `roborepo update` reports what changed (new/changed skills, rules, packages).
- Docs sweep: rewrite `architecture.md`, `harness-anatomy.md`, `harnesses-explained.md`,
  `setup-and-daily-use.md`, `install-workflows.md`, `config-collision-handling.md`,
  `rules-parity-and-layering.md`, `manifest-and-symlinks.md`, `README.md`, and remove
  `managed-adopt-update-installer.md` (or fold to history). Remove managed/adopt/symlink
  language throughout.
- Tests: rewrite `test-install-collisions.sh` (symlink asserts → copy asserts) and

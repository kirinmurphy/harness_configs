---
description: Scan this repo and refresh its Project Context handoff docs (glossary, inventory).
---

<!-- Generated by scripts/build/render-slash-commands.mjs; do not edit directly. -->

# /inventory

Use the `project-context` skill for this request.

Read `~/.claude/skills/project-context/SKILL.md`, then follow its workflow.

Keep the skill as the source of truth; this command is only the explicit entry
point.

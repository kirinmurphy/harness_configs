...
priority: high
next_action: Define and validate the provider manifest, capability vocabulary, discovery result, and installed-harness state schemas before migrating any consumers
blocked_by: []
depends_on: []
related:
...
reviewed_commit: 2651464efc0e80f1176673ce0de63fbdf5d8c76e
---
...
- [ ] Add the provider manifest JSON Schema.
- [ ] Add the capability enum and capability-to-required-method mapping.
- [ ] Define structured action, warning, unsupported, and error result schemas.
- [ ] Define discovery result and persisted harness-state schemas.
- [ ] Add manifest fixtures for Claude, Codex, invalid IDs, invalid capabilities, missing adapter
  bindings, and extension fields.
- [ ] Add app-owned Claude and Codex provider manifests without changing current behavior.
- [ ] Validate provider manifests in `doctor`, packaging verification, and the main test suite.

...
const CURATED_DOCS = ["README.md", "glossary.md", "inventory.md"];
...
export function projectContextCheck(rest) {
  const positional = rest.filter((a) => !a.startsWith("--"));
  const root = resolveTarget(positional[0]);
  const docsDir = path.join(root, DEFAULT_DOCS_DIR);
  const generatedDir = path.join(docsDir, GENERATED_SUBDIR);
  const scanPath = path.join(generatedDir, SCAN_FILE);
  const summaryPath = path.join(generatedDir, SUMMARY_FILE);
  const failures = [];
  ...
  try {
    ...
    failures.push(`missing or invalid generated scan: ${path.relative(root, scanPath)}`);
  }

  if (scan && scan.schemaVersion !== SCHEMA_VERSION) {
    failures.push(`generated scan schema ${scan.schemaVersion}; expected ${SCHEMA_VERSION}`);
  }

  if (scan && typeof scan.fileCount !== "number") {
    failures.push("generated scan missing fileCount");
  }

  if (fs.existsSync(summaryPath)) {
    const summary = fs.readFileSync(summaryPath, "utf8");
    if (!summary.includes("generated by `roborepo project-context inventory --summary`")) {
      failures.push(`summary does not look generated: ${path.relative(root, summaryPath)}`);
    }
  }
  ...
  if (fs.existsSync(docsDir)) {
    for (const doc of CURATED_DOCS) {
      ...
      if (!fs.existsSync(docPath)) failures.push(`missing curated doc: ${path.relative(root, docPath)}`);
    }
  }
  ...
  console.log(`ok: ${path.relative(root, scanPath)}`);
}

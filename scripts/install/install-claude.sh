...
# Render CLAUDE.md from base rule fragments + enabled-packages registry.
if command -v node >/dev/null 2>&1; then
  if [[ "${dry_run}" -eq 1 ]]; then
    node "${repo_root}/scripts/cli/rules-render.mjs" --dry-run claude
  else
    node "${repo_root}/scripts/cli/rules-render.mjs" claude
  fi
fi
